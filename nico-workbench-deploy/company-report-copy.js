...
const latestClosedReportWindow=(now=new Date())=>{const end=new Date(now.getFullYear(),now.getMonth(),now.getDate()),day=end.getDay(),back=day===2?7:(day+5)%7;end.setDate(end.getDate()-back);const start=new Date(end);start.setDate(start.getDate()-6);return{start:localIso(start),end:localIso(end)}};
const followersReminder=r=>{const date=text(r?.publishDate),w=latestClosedReportWindow();return /^20\d{2}-\d{2}-\d{2}$/.test(date)&&date>=w.start&&date<=w.end&&!followersRecorded(r)};
...
window.latestClosedReportWindow=latestClosedReportWindow;
queue();
})();
