...
document.addEventListener('click',e=>{const table=e.target.closest?.('.page')?.querySelector('table')||document.querySelector('table');if(e.target.closest?.('.reportHistoryMode,.excelOnlyFilter,.selectedReportWeek')){reportPage=1;queue();return}const pageBtn=e.target.closest?.('.reportPageBtn');if(pageBtn){reportPage=Number(pageBtn.dataset.page)||1;queue();return}if(e.target.closest?.('.reportPagePrev')){reportPage=Math.max(1,reportPage-1);queue();return}if(e.target.closest?.('.reportPageNext')){reportPage+=1;queue();return}if(table&&e.target.closest?.('.copyCompanyReportBtn'))copyFiltered(table).catch(()=>typeof toast==='function'&&toast('复制失败，请允许浏览器访问剪贴板'))});
