...
const followersRecorded=r=>r?.followersGainedRecorded===true||Number(r?.followersGained||0)!==0;
const followersMetric=r=>followersRecorded(r)?String(Number(r.followersGained||0)):'';
const localIso=d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
const activeFollowersReminderWindow=(now=new Date())=>{const today=new Date(now.getFullYear(),now.getMonth(),now.getDate()),day=today.getDay(),back=(day+4)%7,currentStart=new Date(today);currentStart.setDate(currentStart.getDate()-back);const currentEnd=new Date(currentStart);currentEnd.setDate(currentEnd.getDate()+6);const previousStart=new Date(currentStart);previousStart.setDate(previousStart.getDate()-7);return{previousStart:localIso(previousStart),currentStart:localIso(currentStart),currentEnd:localIso(currentEnd)}};
const followersReminder=r=>{const date=text(r?.publishDate),w=activeFollowersReminderWindow();return /^20\d{2}-\d{2}-\d{2}$/.test(date)&&date>=w.previousStart&&date<=w.currentEnd&&!followersRecorded(r)};
...
const companyRow=r=>[
  text(r.market)||'印尼',text(r.month),text(r.week),text(r.publishDate),text(r.account),text(r.dramaTitle),'微剧',typeText(r),text(r.isIqiyi),text(r.contentDirection),linkText(r),metric(r.views),metric(r.likes),followersMetric(r)
];
...
const paginationRenderKey=(total,totalPages)=>`${total}|${totalPages}|${reportPage}`;
function renderPagination(table){const rows=[...table.querySelectorAll('tbody tr:not(.reportInlineEditor)')],total=rows.length,totalPages=Math.max(1,Math.ceil(total/REPORT_PAGE_SIZE));if(reportPage>totalPages)reportPage=totalPages;const start=(reportPage-1)*REPORT_PAGE_SIZE;rows.forEach((row,i)=>{row.style.display=i>=start&&i<start+REPORT_PAGE_SIZE?'':'none'});const host=table.parentElement||table;let existing=host.querySelector('.reportPagination');if(total<=REPORT_PAGE_SIZE){existing?.remove();return}const renderKey=paginationRenderKey(total,totalPages);if(!existing){existing=document.createElement('div');existing.className='reportPagination';existing.innerHTML='<div class="reportPageSummary"></div><div class="reportPageActions"><button type="button" class="btn reportPagePrev">上一页</button><span class="reportPageNumbers"></span><button type="button" class="btn reportPageNext">下一页</button></div>';host.appendChild(existing)}if(existing.dataset.paginationRenderKey===renderKey)return;existing.dataset.paginationRenderKey=renderKey;existing.querySelector('.reportPageSummary').textContent=`共 ${total} 条 · 第 ${reportPage} / ${totalPages} 页 · 每页 15 条`;existing.querySelector('.reportPagePrev').disabled=reportPage===1;existing.querySelector('.reportPageNext').disabled=reportPage===totalPages;const numbers=existing.querySelector('.reportPageNumbers');const count=numbers.querySelectorAll('.reportPageBtn').length;if(count!==totalPages){numbers.innerHTML=Array.from({length:totalPages},(_,i)=>`<button type="button" class="btn reportPageBtn" data-page="${i+1}">${i+1}</button>`).join('')}numbers.querySelectorAll('.reportPageBtn').forEach(b=>{b.dataset.active=Number(b.dataset.page)===reportPage?'1':'0'})}
...
window.activeFollowersReminderWindow=activeFollowersReminderWindow;
queue();
})();
